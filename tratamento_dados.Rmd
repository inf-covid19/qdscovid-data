...
```{r}
list_dados = c("ac", "al", "ap", "am", "ba-1", "ba-2", "ce", "df", "es", "go-1", "go-2", "ma", "mt", "ms", "mg-1", "mg-2", "mg-3", "pa", "pb", "pr-1", "pr-2", "pe", "pi", "rj-1", "rj-2", "rn", "rs-1", "rs-2", "rs-3", "ro", "rr", "sc-1", "sc-2", "to", "se", "sp-1", "sp-2", "sp-3", "sp-4", "sp-5", "sp-6", "sp-7")
...
```{r}
levels(dados$estadoIBGE)

levels(dados$estadoIBGE)[2] = "Indefinido"
...
```{r}
levels(dados$estadoNotificacaoIBGE)

levels(dados$estadoNotificacaoIBGE)[28] = "Indefinido"
...
## Salvando dados de registros num csv
...
for (num_file in 1:number_files) {
   ...
   write.table(dados[ini:fim,], file = paste("output/qdscovid_registers/qdscovid_registers_", num_file, ".csv", sep = ""), quote = FALSE, row.names = FALSE, sep = ";", eol = "\n")
}
num_file = num_file + 1
ini = fim + 1
fim = dim(dados)[1]
