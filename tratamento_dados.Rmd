...
importCSV <- function(file) {
   ...
   csv = read.csv(file, sep = ";", na.strings = "", stringsAsFactors = T, colClasses = c("character", "character", "character", "NULL", "NULL", "factor", "NULL", "NULL", "factor", "character", "factor", "factor", "NULL", "factor", "NULL", "factor", "NULL", "factor", "NULL", "NULL", "factor", "NULL", "factor", "NULL", "NULL", "character", "character", "factor", "factor")) 
   return(csv)
}
```
...
```{r}
detect = str_detect(unique(dados$municipioIBGE), regex("[0-9][0-9][0-9][0-9][0-9][0-9][0-9]"))
indi = which(!detect)
unique(dados$municipioIBGE)[indi]

levels(dados$municipioIBGE)[23] = "Indefinido"
...
```{r}
unique(dados$estadoNotificacaoIBGE)
...
```{r}
detect = str_detect(unique(dados$municipioNotificacaoIBGE), regex("[0-9][0-9][0-9][0-9][0-9][0-9][0-9]"))
indi = which(!detect)
unique(dados$municipioNotificacaoIBGE)[indi]

levels(dados$municipioNotificacaoIBGE)[324] = "Indefinido"
...
for (num_file in 1:number_files) {
   ...
   write.csv(dados[ini:fim,], file = paste("output/qdscovid_registers_", num_file, ".csv", sep = ""), quote = FALSE, sep = ";", row.names = FALSE)
}
num_file = num_file + 1
ini = fim + 1
fim = dim(dados)[1]
